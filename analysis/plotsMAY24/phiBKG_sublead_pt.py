...
hSGN = df.Define("scale", "w*lumiIntegrated").Define("sub", "getMinimum(goodMeson_trk1_pt, goodMeson_trk2_pt)").Histo1D(("hist", "Subleading track from #phi PT, reconstruction", 70, 0, 70),"sub", "scale")
hBKG = dg.Define("scale", "w*lumiIntegrated").Define("sub", "getMinimum(goodMeson_trk1_pt, goodMeson_trk2_pt)").Histo1D(("hist", "Subleading track from #phi PT, reconstruction", 70, 0, 70),"sub", "scale")
...
stack = ROOT.THStack("stack", "Subleading track from #phi PT, reconstruction")
stack.Add(hBKG.GetValue())
stack.Add(hSGN.GetValue())
stack.Draw("hist")
stack.GetXaxis().SetTitle("p_{T}^{subleadtrk} [GeV]")
...
canvas.SaveAs("~/public_html/plotsMAY24/PhiBKG_sublead_pt.png")
