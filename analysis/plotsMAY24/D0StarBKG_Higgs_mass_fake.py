...
hSGN = df.Define("scale", "w*lumiIntegrated").Histo1D(("m_{H}", "Higgs candidate invariant mass with fake #gamma of p_{T}~5GeV, reconstruction", 80, 0, 200),"HCandMass", "scale")
hBKG = dg.Define("scale", "w*lumiIntegrated").Histo1D(("m_{H}", "Higgs candidate invariant mass with fake #gamma of p_{T}~5GeV, reconstruction", 80, 0, 200),"HCandMass", "scale")
...
stack = ROOT.THStack("stack", "Higgs candidate invariant mass with fake #gamma of p_{T}~5GeV, reconstruction")
...
canvas.SaveAs("~/public_html/plotsMAY24/D0StarBKG_Higgs_mass_fake.png")
