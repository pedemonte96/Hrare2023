...
hSGN = df.Define("scale", "w*lumiIntegrated").Define("leading", "getMaximum(goodMeson_trk1_pt, goodMeson_trk2_pt)").Histo1D(("hist", "Leading track from #phi PT, reconstruction", 70, 0, 70),"leading", "scale")
hBKG = dg.Define("scale", "w*lumiIntegrated").Define("leading", "getMaximum(goodMeson_trk1_pt, goodMeson_trk2_pt)").Histo1D(("hist", "Leading track from #phi PT, reconstruction", 70, 0, 70),"leading", "scale")
...
stack = ROOT.THStack("stack", "Leading track from #phi PT, reconstruction")
stack.Add(hBKG.GetValue())
stack.Add(hSGN.GetValue())
stack.Draw("hist")
stack.GetXaxis().SetTitle("p_{T}^{leadtrk} [GeV]")
...
canvas.SaveAs("~/public_html/plotsMAY24/PhiBKG_lead_pt.png")
