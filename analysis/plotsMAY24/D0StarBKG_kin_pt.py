...
hSGN = df.Define("scale", "w*lumiIntegrated").Histo1D(("hist", "D^{0} PT, reconstruction", 80, 0, 200),"goodMeson_pt", "scale")
hBKG = dg.Define("scale", "w*lumiIntegrated").Histo1D(("hist", "D^{0} PT, reconstruction", 80, 0, 200),"goodMeson_pt", "scale")
...
stack = ROOT.THStack("stack", "D^{0} PT, reconstruction")
stack.Add(hBKG.GetValue())
stack.Add(hSGN.GetValue())
stack.Draw("hist")
stack.GetXaxis().SetTitle("p_{T, 2trk}^{D^{0}#rightarrow K#pi} [GeV]")
...
canvas.SaveAs("~/public_html/plotsMAY24/D0StarBKG_kin_pt.png")
