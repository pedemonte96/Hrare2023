...
hSGN = df.Define("scale", "w*lumiIntegrated").Histo1D(("hist", "#phi kinetic mass, reconstruction", 100, 0, 2),"goodMeson_charged_mass", "scale")
hBKG = dg.Define("scale", "w*lumiIntegrated").Histo1D(("hist", "#phi kinetic mass, reconstruction", 100, 0, 2),"goodMeson_charged_mass", "scale")
...
stack = ROOT.THStack("stack", "#phi kinetic mass, reconstruction")
stack.Add(hBKG.GetValue())
stack.Add(hSGN.GetValue())
stack.Draw("hist")
stack.GetXaxis().SetTitle("m_{2trk}^{#phi#rightarrow #pi#pi} [GeV]")
...
canvas.SaveAs("~/public_html/plotsMAY24/PhiBKG_kin_mass.png")
