...
hSGN = df.Define("scale", "w*lumiIntegrated").Histo1D(("hist", "Pion PT, reconstruction", 50, 0, 100),"goodMeson_trk1_pt", "scale")
hBKG = dg.Define("scale", "w*lumiIntegrated").Histo1D(("hist", "Pion PT, reconstruction", 50, 0, 100),"goodMeson_trk1_pt", "scale")
...
stack = ROOT.THStack("stack", "Pion PT, reconstruction")
stack.Add(hBKG.GetValue())
stack.Add(hSGN.GetValue())
stack.Draw("hist")
stack.GetXaxis().SetTitle("p_{T}^{#pi} [GeV]")
...
canvas.SaveAs("~/public_html/plotsMAY24/D0StarBKG_pion_kin_pt.png")
