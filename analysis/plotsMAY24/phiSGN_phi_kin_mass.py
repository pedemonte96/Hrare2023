...
h0=df.Define("scale", "w*lumiIntegrated")\
    ...
    .Define("good", "goodMeson_charged_mass[filt]")\
    .Histo1D(("hist", "#phi kinematic mass", bins, 0.0, 2.0),"good")
...
legend = ROOT.TLegend(0.57, 0.65, 0.8999, 0.89)
legend.SetMargin(0.17)
legend.SetBorderSize(0)
legend.SetTextSize(0.04)
legend.AddEntry(h0.GetValue(), "0 #gamma (mean: {:.3f} GeV)".format(round(h0.GetMean(), 3)), "f")
legend.AddEntry(h1.GetValue(), "1 #gamma (mean: {:.3f} GeV)".format(round(h1.GetMean(), 3)), "f")
legend.AddEntry(h2.GetValue(), ">2 #gamma (mean: {:.3f} GeV)".format(round(h2.GetMean(), 3)), "f")
legend.Draw()

canvas.SaveAs("~/public_html/plotsMAY24/PhiSGN_phi_kin_mass.png")
