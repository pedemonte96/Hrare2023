...
def getHisto(nbin, xlow, xhigh, date, nums, cat, mesonCat, mesonLatex, year, filters=[], extraTitle=None, ditrack=False, regModelName=None):
    ...
    if regModelName is None:
        ...
        s = '''
        TMVA::Experimental::RReader {variableName}Reader0("/data/submit/pdmonte/TMVA_models/weightsOptsFinalBis/TMVARegression_{modelName}_{channel}_{prodCat}_0.weights.xml");
        {variableName}0 = TMVA::Experimental::Compute<{numVarsTotal}, float>({variableName}Reader0);
        TMVA::Experimental::RReader {variableName}Reader1("/data/submit/pdmonte/TMVA_models/weightsOptsFinalBis/TMVARegression_{modelName}_{channel}_{prodCat}_1.weights.xml");
        {variableName}1 = TMVA::Experimental::Compute<{numVarsTotal}, float>({variableName}Reader1);
        TMVA::Experimental::RReader {variableName}Reader2("/data/submit/pdmonte/TMVA_models/weightsOptsFinalBis/TMVARegression_{modelName}_{channel}_{prodCat}_2.weights.xml");
