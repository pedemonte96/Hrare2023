...
if [[ $# == 0 || $# -gt 2 ]]; then
    ...
    if [[ ${1,,} == "f" || ${1,,} == "false" ]]; then
        ...
        if [[ $# == 1 ]]; then
            outputFile="/data/submit/pdmonte/TMVA_models/finalEvalFilesPhi/eval_tmp.out" 
        elif [[ $2 =~ ^[0-9]+$ ]]; then
            outputFile="/data/submit/pdmonte/TMVA_models/finalEvalFilesPhi/eval_${2}vars.out"
            filtering=1
        else
        ...
        echo -e "First parameter t (output at /data/submit/pdmonte/TMVA_models/finalEvalFilesPhi/) or f (output at eval_all_phi.out)."
        exit 1
    fi
fi
