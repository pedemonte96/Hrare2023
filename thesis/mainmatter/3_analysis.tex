...
\begin{myitemlist}
    ...
    \todo{Present the models, and attatch hyperparameters in the appendix. For each channel plot pt generation, reco and corrected + residuals corrected and reco.}

    \todo{explain that to validate the model we have done cross-validation, training on AB and testing on C, rotating.}
    ...
    \begin{table}[!ht]
        ...
        \begin{tabular}{|l|c|C{3.1cm}@{}c|}
            \hline
            \cellcolor{lightgray}Decay channel & \cellcolor{lightgray}MSE without $\pT$ regression & \multicolumn{2}{c|}{\cellcolor{lightgray}MSE with $\pT$ regression} \\ \hline
            $\phi$          &37.8 MeV   &34.0 MeV  & (-10\%)   \\
